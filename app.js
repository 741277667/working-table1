...
function render(){
  ...
  $$('[data-module-add]').forEach(b=>b.onclick=()=>newPaper(view,b.dataset.moduleAdd));
...
function startDrag(e,el,kind){
  if(e.button!==0||drag||e.target.closest('button:not(.zone-title):not(.module-title),input,textarea,select,.zone-menu')||el.dataset.ghost==='true')return;
...
function iconButton(el,name,label){if(!el)return;el.innerHTML=`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${paths[name]||paths.plus}"/></svg>`;el.title=label;el.setAttribute('aria-label',label);el.classList.add('icon-button');}
function polishButtons(){if(drag)return;
  $$('[data-action]').forEach(el=>iconButton(el,glyphs[el.dataset.action],({today:'拿到今日托盘',home:'放回原位',waiting:'等待',done:'完成并归档',archive:'归档',duplicate:'复制',unstack:'移出纸堆',trash:'丢入废纸篓',destroy:'永久删除',restore:'恢复任务'}[el.dataset.action])));
  $$('[data-drawer="trash"]').forEach(el=>iconButton(el,'trash','废纸篓'));
  iconButton($('#backup'),'download','导出备份');iconButton($('#back'),'back','返回桌面');iconButton($('#move-confirm'),'move','移动');
  $$('.close,[data-remove-check]').forEach(el=>iconButton(el,'close','关闭 / 移除'));
  $$('[data-delete-module]').forEach(el=>iconButton(el,'trash','删除文件夹'));
  $$('[data-module-add],#quick-new,#add,#inbox-add').forEach(el=>iconButton(el,'plus','新建纸条'));
  $$('[data-complete]').forEach(el=>{el.title=el.getAttribute('aria-label');});
}
